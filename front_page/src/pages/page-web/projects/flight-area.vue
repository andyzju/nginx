<template>
  <div class="project-flight-area-wrapper height-100">
    ...
    >
      ...
      <FlightAreaPanel
        :data="flightAreaList"
        @location-area="clickArea"
        @delete-area="deleteAreaById"
      />
      <DividerLine />
      <FlightAreaSyncPanel />
    </a-spin>
...
const deleteArea = (area: FlightAreaUpdate) => {
  flightAreaList.value = flightAreaList.value.filter(
    (data) => data.area_id !== area.area_id
  )
  useGMapCoverHook.removeCoverFromMap(area.area_id)
}

const updateArea = (area: FlightAreaUpdate) => {
  flightAreaList.value = flightAreaList.value.map((data) =>
    data.area_id === area.area_id ? area : data
  )
