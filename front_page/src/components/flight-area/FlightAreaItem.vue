<template>
  <div
    ...
    :class="{ disable: !flightArea.status }"
  >
    <div class="title">
    ...
    <div class="mt5 ml10" style="color: hsla(0, 0%, 100%, 0.35)">
      <span class="mr10"
        >更新时间
        {{ formatDateTime(flightArea.update_time).toLocaleString() }}</span
      >
    </div>
    <div
      class="flex-row flex-justify-between flex-align-center ml10 mt5"
      style="color: hsla(0, 0%, 100%, 0.65)"
    >
      <FlightAreaIcon
        :type="flightArea.type"
        :isCircle="EGeometryType.CIRCLE === flightArea.content.geometry.type"
      />
      <div class="mr10 operate">
        <a-popconfirm
          v-if="flightArea.status"
          title="Is it determined to disable the current area?"
          okText="Disable"
        ...
        <a-popconfirm
          v-else
          @confirm="changeAreaStatus(true)"
          title="Is it determined to enable the current area?"
          okText="Enable"
        >
        ...
        <a-popconfirm
          title="Is it determined to delete the current area?"
          okText="Delete"
          okType="danger"
          @confirm="deleteArea"
        >
