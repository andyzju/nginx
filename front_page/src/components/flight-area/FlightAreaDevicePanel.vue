    ...
    <div class="scrollbar">
      <div id="data" v-if="data.length !== 0">
        <div v-for="dock in data" :key="dock.device_sn">
          ...
          >
            <div style="width: 88%">
              ...
              <div
                class="ml10 mr10 pr5 pl5 flex-align-center flex-row flex-justify-between"
                style="background: #595959"
              >
                <div>Custom Flight Area</div>
                <div>
                  ...
                  <div
                    v-else-if="
                      deviceStatusMap[dock.device_sn]?.flight_area_status
                        ?.sync_status === ESyncStatus.SYNCHRONIZED
                    "
                  >
                  ...
                  <div
                    v-else-if="
                      deviceStatusMap[dock.device_sn]?.flight_area_status
                        ?.sync_status === ESyncStatus.SYNCHRONIZING ||
                      deviceStatusMap[dock.device_sn]?.flight_area_status
                        ?.sync_status === ESyncStatus.WAIT_SYNC
                    "
                  >
        ...
        <div class="flex-row flex-justify-between footer">
          <a-button class="mr10" @click="closePanel">Cancel </a-button>
          <a-button
            type="primary"
            :disabled="confirmDisabled"
            @click="syncDeviceFlightArea"
            >Sync
          </a-button>
        </div>
      </div>
...
const deviceStatusMap = computed(() =>
  props.data.reduce(
    (obj: Record<string, GetDeviceStatus>, val: GetDeviceStatus) => {
...
const getDocks = async () => {
  await getBindingDevices(workspaceId, body, EDeviceTypeName.Dock).then(
    (res) => {
      if (res.code !== 0) {
        return
      }
...
const selectDock = (dock: Device) => {
  ...
  if (
    deviceStatusMap.value[dock.device_sn]?.flight_area_status?.sync_status ===
      ESyncStatus.SYNCHRONIZING ||
    deviceStatusMap.value[dock.device_sn]?.flight_area_status?.sync_status ===
      ESyncStatus.WAIT_SYNC
  ) {
    message.info('The dock is synchronizing.')
    return
  }
  selectedDocksMap.value[dock.device_sn] =
    !selectedDocksMap.value[dock.device_sn]
}
...
onMounted(() => {
  ...
  const key = setInterval(() => {
    if (
      body.total === 0 ||
      Math.ceil(body.total / body.page_size) <= body.page
    ) {
      clearInterval(key)
      return
    }
