<template>
  <div class="flight-area-sync-panel p10 flex-row flex-align-center">
    ...
    <div class="ml20 mr10 flex-column" @click="switchPanel">
      <div class="fz18">跨设备同步</div>
      <div v-if="syncDevicesCount > 0">
        <a-spin /> Syncing to {{ syncDevicesCount }} devices
      </div>
    </div>
    <RightOutlined class="fz18" @click="switchPanel" />
    <FlightAreaDevicePanel
      v-if="visible"
...
const syncDevicesCount = computed(
  () =>
    syncDevices.value.filter(
      (device) =>
