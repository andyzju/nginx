  ...
  <div class="create-plan-wrapper">
    ...
    <div class="content">
      ...
      >
        ...
        <!-- 任务类型 -->
        <a-form-item label="任务类型" class="plan-timer-form-item">
        ...
        <a-form-item
          label="日期"
          v-if="
            planBody.task_type === TaskType.Timed ||
            planBody.task_type === TaskType.Condition
          "
        ...
        >
          <div
            class="mb10 flex-row flex-align-center flex-justify-around"
            v-for="n in planBody.select_time_number"
            :key="n"
          >
            <a-time-picker
        ...
        <template v-if="planBody.task_type === TaskType.Condition">
          ...
          <!-- storage capacity -->
          <a-form-item
            label="当存储量达到时开始任务 (MB)"
            :labelCol="{ span: 23 }"
            name="storage_capacity"
          >
            <a-input-number
              v-model:value="planBody.min_storage_capacity"
              class="width-100"
            >
            </a-input-number>
          </a-form-item>
        ...
        >
          <div style="white-space: nowrap">
            <a-radio-group
              v-model:value="planBody.out_of_control_action"
              button-style="solid"
            >
              <a-radio-button
  ...
  valueRef.value
    ...
    .then(() => {
      ...
      if (planBody.select_execute_date.length === 2) {
        ...
        for (
          let i = planBody.select_execute_date[0];
          i.isSameOrBefore(planBody.select_execute_date[1]);
          i.add(1, 'days')
        ) {
      ...
      if (
        wayline.value &&
        wayline.value.template_types &&
        wayline.value.template_types.length > 0
      ) {
